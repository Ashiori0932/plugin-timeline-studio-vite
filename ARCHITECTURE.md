...
`project.timeline.transitionDuration` 已进入数据模型和示例文件，但当前展示运行时没有实现淡入、滑动或其他切换动画。

实现过渡时，不建议只在 CSS 中读取固定值。应明确区分：

- 当前项播放时间。
- 离场项与入场项。
- 过渡占用的时间是否包含在 `duration` 内。
- 手动切换、暂停和快速连续切换的行为。
...
| `transitionDuration` 未使用 | 配置不会产生视觉过渡 | 在运行时设计明确的过渡状态机 |
